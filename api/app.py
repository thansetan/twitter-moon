...
async def picture(user: User,response: Response):
    try:
        ...
        return APIResponse(
                            ...
                            errors=[f"request to {timeout.__cause__.request.url} timed out after {timeout.duration:.2f} s"],
        )
    except errors.HTTPException as http:
            logging.exception("error while trying to update the profile picture", exc_info=http)
            response.status_code = 500
            return APIResponse("there's an error", errors=http.api_errors if http.api_errors else [str(http)])
    except Exception as e:
        logging.exception("error while trying to update the profile picture", exc_info=e)
        response.status_code = 500
